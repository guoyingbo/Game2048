...
bool NumberCell::init()
{
    ...
    ccColor4B color;
    if (m_number < 12)
        color = m_color[m_number-1];
    else
        color = m_color[10];
    float fontSize = m_size.height/2;
    if (m_number > 9)
        fontSize *= 0.75;
    if (m_number > 13)
        fontSize *= 0.85;
    ...
    CCLabelTTF *pLable = CCLabelTTF::create(snum.getCString(),"Arial Bold",fontSize);
